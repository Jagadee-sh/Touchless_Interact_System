...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            hand_size = np.sqrt((wrist_pos[0] - middle_base_pos[0])**2 + 
                              (wrist_pos[1] - middle_base_pos[1])**2)
    ...
    if keyboard_active:
        ...
        for i, row in enumerate(keyboard_layout):
            for j, key in enumerate(row):
                ...
                if key == " ":
                    ...
                    cv2.rectangle(overlay, (x, y), (x + space_width, y + key_size), 
                                (200, 200, 200, 128), -1)
                    cv2.rectangle(overlay, (x, y), (x + space_width, y + key_size), 
                                (100, 100, 100), 2)
                    cv2.putText(overlay, "SPACE", (x + space_width//4, y + 35), 
                              cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
                else:
                    cv2.rectangle(overlay, (x, y), (x + key_size, y + key_size), 
                                (200, 200, 200, 128), -1)
                    cv2.rectangle(overlay, (x, y), (x + key_size, y + key_size), 
                                (100, 100, 100), 2)
                    cv2.putText(overlay, key, (x + 15, y + 35), 
        ...
        if right_hand_index_tip and right_hand_thumb_tip and typing_cooldown == 0:
            distance = np.sqrt((right_hand_index_tip[0] - right_hand_thumb_tip[0])**2 + 
                             (right_hand_index_tip[1] - right_hand_thumb_tip[1])**2)
            if distance < 30:
                for i, row in enumerate(keyboard_layout):
                    for j, key in enumerate(row):
                        x = keyboard_origin[0] + j * key_size
                        y = keyboard_origin[1] + i * key_size
                        if key == " ":
                            if (x <= right_hand_index_tip[0] <= x + 5 * key_size and 
                                y <= right_hand_index_tip[1] <= y + key_size):
                                if last_key != " ":
                                    pyautogui.typewrite(" ")
                                    last_key = " "
                                    typing_cooldown = 20
                            ...
                            if (x <= right_hand_index_tip[0] <= x + key_size and 
                                y <= right_hand_index_tip[1] <= y + key_size):
                                if key != last_key:
